...
export default function App() {
  ...
  const openPinecilConnection = useCallback(async (device: Device) => {
    try {
      ...
      const targetService = services.find((service) =>
        service.uuid === BULK_SERVICE_UUID
      );
      ...
      const id = setInterval(async () => {
        try {
          ...
          const buffer = Uint8Array.from(decodedBytes, (c) => c.charCodeAt(0)).buffer;
          const dataView = new DataView(buffer);

          const temperature = dataView.getUint32(0, true);
          const setpoint = dataView.getUint32(4, true);
          const inputVoltage = dataView.getUint32(8, true) / 10;
          const handleTemperature = dataView.getUint32(12, true) / 10;
          // const maxTemperature = dataView.getUint32(36, true);
          const powerWatts = dataView.getUint32(52, true) / 10;
  ...
  const handleSetpointChange = useCallback(async (setpoint: number) => {
    try {
      ...
      const targetService = services.find((service) =>
        service.uuid === SETTINGS_SERVICE_UUID
      );
      ...
      if (!targetChar) {
        throw new Error(
          `Characteristic ${SETTINGS_SETPOINT_CHAR_UUID} not found`,
        );
      }
      ...
      await targetChar.writeWithResponse(
        base64.encode(String.fromCharCode.apply(null, [...new Uint8Array(view.buffer)])),
      );
    } catch (error) {
